...
export default function LoginPage() {
  ...
  return <AuthLayout title="Bienvenue" subtitle="Connectez-vous à l’espace de votre agence." footer={<>Pas encore de compte ? <Link to="/register">Créer mon agence</Link></>}><form onSubmit={handleSubmit(submit)} noValidate>{message && <p className="form-error">{message}</p>}<label>Email professionnel<input type="email" placeholder="agence@exemple.sn" {...register('email')} />{errors.email && <small className="field-error">{errors.email.message}</small>}</label><label>Mot de passe<input type="password" placeholder="Votre mot de passe" {...register('password')} />{errors.password && <small className="field-error">{errors.password.message}</small>}</label><Button disabled={isSubmitting} type="submit">{isSubmitting ? 'Connexion…' : 'Se connecter'}</Button></form></AuthLayout>
}
